...
def run_one_fold(args, hparams, loader, split_manifest, fold_idx):
    fold_dir = os.path.join(args.out_dir, hparams.data_name, f'fold_{fold_idx}')
    os.makedirs(fold_dir, exist_ok=True)
    train_loader, val_loader, _test_loader, split_meta = loader.get_cv_loaders_from_manifest(split_manifest, fold_idx)
    logging.info(
    ...
    coarse_adj, coarse_meta = coarse_graph.build_coarse_adjacency(
        ...
        active_subgraph_ids=active_subgraph_ids(loader),
        top_k=int(args.top_k),
        include_self=False,
        symmetrize=True,
    )
    coarse_path = os.path.join(fold_dir, 'coarse_adj.npz')
    coarse_graph.save_coarse_adjacency(coarse_path, coarse_adj, coarse_meta)
...
def main():
    ...
    for fold_idx in fold_indices:
        ...
        results.append(run_one_fold(args, fold_hparams, loader, split_manifest, fold_idx))
    if len(results) > 1:
