...
def _extract_batch_pos_probs(logits, batch_size):
    if logits is None:
        return [None] * batch_size
    if logits.dim() == 2 and logits.size(1) == 2:
        return torch.softmax(logits, dim=1)[:, 1].detach().cpu().tolist()
...
def export_lightweight_attention_from_model(
    ...
    with torch.no_grad():
        ...
        for raw_graph_data in export_loader:
            ...
            batch_probs = _extract_batch_pos_probs(logits, len(num_list))
            weights_list, _ = _extract_lightweight_attention(model, out, num_list)
            ...
            for idx, num_nodes in enumerate(num_list):
                ...
                if label_int == 1:
                    top1_pos_count = int(np.sum(node_labels[order[:1]] == 1))
                    top3_pos_count = int(np.sum(node_labels[order[:min(3, num_nodes)]] == 1))
                    top5_pos_count = int(np.sum(node_labels[order[:min(5, num_nodes)]] == 1))
                    stats['pos_bag_for_hit'] += 1
                    stats['top1_hit_sum'] += int(top1_pos_count > 0)
                    stats['top3_hit_sum'] += int(top3_pos_count > 0)
                    stats['top5_hit_sum'] += int(top5_pos_count > 0)
                ...
                if current_pos in selected_positions:
                    per_graph_rows.append({
                        ...
                        'top1_hit_pos': top1_pos_count,
                        'top3_hit_pos': top3_pos_count,
                        'top5_hit_pos': top5_pos_count,
                        'attention_max': float(np.max(weights)),
                        'attention_q75': float(np.quantile(weights, 0.75)),
                        'attention_q50': float(np.quantile(weights, 0.50)),
                        'attention_q25': float(np.quantile(weights, 0.25)),
                        'attention_min': float(np.min(weights)),
